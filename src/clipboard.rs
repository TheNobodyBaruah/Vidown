...
/// 2. If `arboard` fails or is unavailable (e.g. WSL without active X server, or headless session):
...
pub fn sanitize_clipboard_text(text: &str) -> String {
    ...
    // Strip enclosing single or double quotes (e.g. copied from command-line or markdown)
